...
- **数据源抽象层**（`quantify/backtest/datasource.py`）：引擎不再硬编码 ETF 表，按代码自动路由：
  - `EtfDataSource` → `fund_daily`/`fund_adj`/`fund_nav`/`fund_div`（份额折算比例 = `adj_factor` 跳变率 / `(accum_nav/unit_nav)` 跳变率，分离现金分红与份额折算；无 NAV 数据时默认 1.0 不折算；±0.5% 以内视为噪声）
